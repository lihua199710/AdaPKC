...
from adapkc.utils import MVRSS_HOME
from adapkc.losses.soft_dice import SoftDiceLoss
from adapkc.losses.coherence import CoherenceLoss
from adapkc.losses.soft_coherence import SoftCoherenceLoss
from adapkc.losses.sparse_coherence import SparseCoherenceLoss
from adapkc.losses.smospa_coherence import SmoSpaCoherenceLoss
from adapkc.losses.distribution_coherence import DistributionCoherenceLoss
from adapkc.losses.denoise_coherence import DenoiseCoherenceLoss
from adapkc.loaders.dataloaders import Rescale, Flip, HFlip, VFlip
...
def define_loss(signal_type, custom_loss, device):
    ...
    # zlw@20231016
    elif custom_loss == 'wce_w10sdice_w5col_track':
        weights = get_class_weights(signal_type)
        ce_loss = nn.CrossEntropyLoss(weight=weights.to(device).float())
        loss = [ce_loss, SoftDiceLoss(global_weight=10.), CoherenceLoss(global_weight=5.), ce_loss, CoherenceLoss(global_weight=1.)]
    # zlw@20220302
