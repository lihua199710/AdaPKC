...
from adapkc.utils.functions import transform_masks_viz, get_metrics, normalize, define_loss, get_transformations, get_qualitatives
from adapkc.utils.paths import Paths
from adapkc.utils.metrics import Evaluator
from adapkc.loaders.dataloaders import CarradaDataset
from adapkc.utils.distributed_utils import reduce_value, get_rank, all_gather
        ...
        with torch.no_grad():
            for i, sequence_data in enumerate(seq_loader):
                ...
                carrada_dataset = CarradaDataset(seq,
                                                ...
                                                add_temp)
                if self.distributed:
                    sampler_test = torch.utils.data.distributed.DistributedSampler(carrada_dataset)
                else:
                    sampler_test = torch.utils.data.SequentialSampler(carrada_dataset)
                ...
                for j, frame in enumerate(frame_dataloader):
                    ...
                    ra_metrics.add_batch_dist(torch.argmax(ra_mask, axis=1),
                                         torch.argmax(ra_outputs, axis=1))
                    if nb_losses < 3:
                        ...
                        ra_losses = reduce_value(ra_losses)
                        ra_loss = reduce_value(ra_loss)
                        ...
                        ra_losses = reduce_value(ra_losses)
                        ra_loss = reduce_value(ra_loss)
                        coherence_loss = reduce_value(coherence_loss)
                        loss = reduce_value(loss)
