...
class PeakConv2D(nn.Module):
    ...
    def _set_lr(module, grad_input, grad_output):
        # only change z/w and z/b, not change z/x
        grad_input_temp = []
        for i in range(len(grad_input)):
            if i == 0 or grad_input[i] is None:
                grad_input_temp.append(grad_input[i])
            else:
                grad_input_temp.append(grad_input[i] * 0.1)
    ...
    def _get_p_r(self, b, h, w, N, dtype):
        ...
        p_c = self._get_p_c(b, h, w, N, dtype)
        # (B, 2N, h, w)
        # the relative position of batch is shared.
        p_r = p_c + prf
        return p_r
