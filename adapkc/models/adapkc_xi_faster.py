# Accelerated AdaPKC-Xi
import torch
import torch.nn as nn
import torch.nn.functional as F
from adapkc.utils.correlation import Correlation
...
class AdaPKC_Xi_Faster(nn.Module):
    """ 
    Accelerated Version of Metric-based Adaptive Peak Convolution RSS Network (accelerated AdaPKC-Xi)
...
if __name__ == '__main__':
    from adapkc.models.adapkc_xi import AdaPKC_Xi
    ori_net = AdaPKC_Xi(n_classes=4, n_frames=5)
    fast_net = AdaPKC_Xi_Faster(n_classes=4, n_frames=5)
    
    # keep the initial weights consistent
    ori_net.load_state_dict(fast_net.state_dict(), strict=False)
    
    device = torch.device('cuda:0')
    ori_net.to(device)
    fast_net.to(device)
    rd_input = torch.randn(1, 1, 5, 256, 64).to(device)
    ra_input = torch.randn(1, 1, 5, 256, 256).to(device)
    ad_input = torch.randn(1, 1, 5, 256, 64).to(device)
    ori_rd_output, ori_ra_output = ori_net(rd_input, ra_input, ad_input)
    fast_rd_output, fast_ra_output = fast_net(rd_input, ra_input, ad_input)
    
    print('mean error in rd output: ', (ori_rd_output-fast_rd_output).mean())
    print('mean error in ra output: ', (ori_ra_output-fast_ra_output).mean())
